...
export function AddOns({ translations }) {
    ...
    return (
        <article className="rounded-lg bg-gradient-to-br from-[#717892]/10 to-transparent border border-[#717892] px-8 pt-12 pb-6 mx-auto max-w-4xl relative overflow-hidden">
            <motion.h2 {...animationProps} className="text-center mx-auto text-2xl md:text-4xl text-textPrimary">{translations.t3_1} <span className="bg-gradient-to-br from-[#0052B4] to-[#45D1FF] bg-clip-text text-transparent">{translations.t3_2}</span></motion.h2>
            ...
            <ul className="flex flex-col divide-y divide-[#717892] max-w-sm">
                {
                    addons.map(({ title, desc }) => (
                        <li key={title} className="flex flex-col gap-4 py-6">
                            <motion.strong {...animationProps} className="font-medium text-textPrimary">{title}</motion.strong>
