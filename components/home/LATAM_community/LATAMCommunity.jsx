...
export function LATAMCommunity() {
    ...
    const videoSources = [
        '/videos/LATAMvideo1.mp4',
        '/videos/LATAMvideo2.mp4',
        '/videos/LATAMvideo3.mp4',
    ];

    return (
        <SectionLayout padding bgColor='bg-[#0B111D] text-[#A2ACCF]'>
            <h3 className='text-white font-light text-3xl max-w-[350px] mb:max-w-full mb:text-4xl text-center '>
                Escuchalo de nuestra Comunidad
            </h3>

            <div className='flex items-center justify-center gap-6 pb-4 mt-10 overflow-x-auto md:mt-20'>
                {
                    videoSources.map((video, index) => (
                        <div key={index} className='group relative rounded-lg overflow-hidden aspect-[9/16] w-64 min-w-[256px] border border-[#717892] grid place-items-center'>
                            <video autoPlay loop muted={soundEnabledIndex !== index} playsInline preload="metadata" className='w-full pointer-events-none' src={video} />

                            <button onClick={() => handleVideoClick(index)} className="absolute transition-all cursor-pointer md:opacity-0 md:translate-y-6 group-hover:opacity-100 group-hover:translate-y-0 top-2 right-2">
                                {
